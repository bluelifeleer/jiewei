$(function(){
  ...
  $(document).on('pageInit','#page-cart',function(e,id,page){
    ...
    // if(!userInfo){
    //   $.router.loadPage("login.php?backPage=cart.php");  //转到登录页
    // }
    ///////////////////////
    //判断是否的登入--------END //
    ///////////////////////
    ...
    if(cartInfo){
      ...
      $.each(cartInfo,function(index, el) {
        ...
        if(el.userid == userid&& el.siteid == siteid ){
          ...
          html += '<a class="c666 external" href="product.php?id='+el.id+'">';
          html += '<img class="w80 pos-r" style="top:50%;" src="'+el.thumb+'" alt="">';
          html += '</a>';
          html += '</div>';
          html += '<div class="w50b fs06 fll">';
          html += '<div class="padl10 padr10">';
          html += '<a class="c666 external fs06" href="product.php?id='+el.id+'">'+cutString(el.title,40)+'</a>';
          html += '</div>';
    ...
    $(page).on('click','#submit_order',function(){
      ...
      if(!isSelect){
        ...
        $.router.loadPage("submit_order.php");  //转到结算页面
