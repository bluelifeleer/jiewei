...
function product_add_Items(offset,pages,siteid,fromid) {
        ...
        $.ajax({
                ...
                url: url,
                async: false,
                data: {'offset':offset,'pages':pages,siteid:siteid,fromid:fromid},
                dataType: "json",
                ...
                success: function(result){
                      if(result.code == 0){
                        ...
                        for(var i=0; i < result.data.length; i++){
                          ...
                          var url = result.data[i].fromid != null ? 'shop_product_detail_manage.php?id='+result.data[i].id:'shop_product_create.php?id='+result.data[i].id;
                ...
                success:function(res){
                  ...
                  if(res.code == 0){
                    $.confirm('删除成功是否去添加商品？',function(){
                      ...
                      $.router.loadPage('shop_product_add.php',true);
                    },function(){
                      $('#product-list-'+productId).remove();
                      return false;
                    });
