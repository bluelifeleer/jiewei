 ...
 ***********************************************************************************/
require_once ('initphp.conf.php');
 //导入框架配置类
require_once ('init/core.init.php');
 //导入核心类文件
require_once ('init/exception.init.php');
 //导入核心类文件
require_once ('init/interceptorInterface.init.php');
//注：该参数必须在sesson_start()之上，
 //导入拦截器接口
define("ERROR", "ERROR");
define("WARN", "WARN");
define("DEBUG", "DEBUG");
...
class InitPHP extends coreInit
{
    ...
    public static function init() {
        ...
        try {
            ...
            require (INITPHP_PATH . '/init/interceptor.init.php');
             //拦截器
            InitPHP::loadFunction("global", '', true);
    ...
    public static function rpc_init() {
        ...
        if (!isset($InitPHP_conf['provider']['allow']) || !in_array($fullClass, $InitPHP_conf['provider']['allow'])) {
            return InitPHP::rpc_ret(405, $fullClass."This class is not allow to access");
        }
     ...
     * @param $data 错误内容
     */
    public static function Encode($code, $msg, $data = null, $callback = false) {
        $ret = array();
        $ret['code'] = $code;
        $ret['msg'] = $msg;
        $ret['data'] = $data;
     ...
     */
    public static function loadFunction($func, $path = '', $base = false) {
        $InitPHP_conf = InitPHP::getConfig();
        $path = ($path == '') ? '' : $path . '/';
        if ($base) {
            $file = rtrim(INITPHP_PATH, '/') . '/' . rtrim($InitPHP_conf['function']['path'], '/') . '/' . $path . $func . '.php';
        }
        else {
            $file = rtrim($InitPHP_conf['function']['path'], '/') . '/' . $path . $func . '.php';
        }
        if (file_exists($file)) {
            include $file;
        }
    ...
    public static function getRemoteService($class, $method, $args = array(), $group = "default", $path = "", $timeout = 5) {
        ...
        require_once ($classPath);
        $curl = new curlInit();
        $temp = array("class" => $class, "method" => $method, "args" => $args, "path" => $path);
    ...
    public static function acl($config = array()) {
        ...
        if ($InitPHP_conf['ismodule']) {
            ...
            if (isset($config[$c]) && in_array($a, $config[$c])) return true;
            return false;
        }
    }
    ...
    public static function getController($controllerName, $functionName, $params = array(), $controllerPath = '') {
        ...
        if (!$controller) return InitPHP::initError('can not loadclass : ' . $controllerName);
        if (!method_exists($controller, $functionName)) return InitPHP::initError('function is not exists : ' . $controllerName);
        if (!$params) {
