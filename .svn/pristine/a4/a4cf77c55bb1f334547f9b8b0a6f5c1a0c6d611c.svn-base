...
class dbInit extends sqlbuildInit
{
    ...
    /**
     * SQL操作-根据id范围更新数据
     * DAO中使用方法：$this->dao->db->update_by_in_field($data,  $field, $ins,  $table_name)
     * @param  array  $data 参数
     * @param  string $field 字段参数
     * @param  array  $ins 字段参数范围
     * @param  string $table_name 表名
     * @return bool
     */
    public function update_by_in_field($data, $field, $where, $table_name)
    {
        ...
        $where = $this->build_in($where);
        $data  = $this->build_update($data);
        $sql   = sprintf("UPDATE %s %s WHERE %s  %s", $table_name, $data, $field, $where);
        return $this->query($sql);
    }
     ...
     */
    public function get_all($table_name, $num = 20, $offest = 0, $field = array(), $id_key = 'id', $sort = 'DESC', $key = false, $fields = '*')
    {
        if (is_array($field)) {
            $where = $this->build_where($field);
        } else {
            if (trim($field) != '') {
                // 波哥 我修改啦
                $field = ltrim($field, ' WHERE');
                $where = ' WHERE ' . ltrim($field, ' WHERE');
            }

        }

        $limit = $this->build_limit($offest, $num);
        $sql   = sprintf("SELECT %s FROM `%s` %s ORDER BY `%s` %s %s", $fields, $table_name, $where, $id_key, $sort, $limit);
        //return $sql;
        $result = $this->query($sql, false);
