		   			...
		   			if(res.code == 0){
		   				if(parseInt(res.data[1]) == 0){
			   				...
			   				for(var i=0; i<res.data[0].length; i++){
			   					...
			   					var  order  = '';
			   					if(res.data[0][i]['orderid']){
									order = 	'<div >'+res.data[0][i]['orderid']+'</div>';

			   					}
			   					htmlStr += '<li class="item-content borb1 fs06 bce6 padt5 padb15">'+
									'<div class="fll w70b textove">'+
										'<div>'+res.data[0][i]['create_time']+'</div>'+
										order+
										'<div >'+res.data[0][i]['title']+'</div>'+
									'</div>'+
									'<div class="flr"  style="line-height:3rem;">'+
			   				...
			   				$.hideIndicator();
			   				$('.list-container > ul').append(htmlStr);
		   				}
