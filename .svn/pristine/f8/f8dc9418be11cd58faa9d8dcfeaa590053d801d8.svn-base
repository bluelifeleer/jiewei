...
class shopOrderController extends Controller {
    ...
    */
    public function delivery() {
        ...
        if(!isset($shopid) || !isset($orderid) || !isset($goodsid)) InitPHP::Encode(1, 'Error', '缺少必要参数');
        // 发货时间
        $year=((int)substr($data['shipping_time'],0,4));
        $month=((int)substr($data['shipping_time'],5,2));
        $day=((int)substr($data['shipping_time'],8,2));
        $shippingInfo['shipping_time'] = mktime(0,0,0,$month,$day,$year);
        $shippingInfo['shipping_name'] = $data['shipping_name'];
        $shippingInfo['shipping_no'] = $data['shipping_no'];

        // 商家确认发货
        $orderDelivery = InitPHP::getRemoteService('orderGoods', 'confirmDelivery', array($orderid,$goodsid,$shopid,$shippingInfo));
        // var_dump(expression)
        if($orderDelivery){
            $to_userid = $orderDelivery['userid'];
            $contents = '您的产品'.$orderDelivery['goods_name'].',于'.$data['shipping_time'].'发货,'.$data['shipping_name'].'快递，单号：'.$data['shipping_no'];
            // var_dump($orderDelivery['data']);
            $message = InitPHP::getRemoteService('message', 'send_one', array($to_userid,$contents,2));
            if($message){
                 InitPHP::Encode(0, 'Success', '');
           }
        }else{
            InitPHP::Encode(1, 'Error', '发货失败');
        }
        //向js返回数据
