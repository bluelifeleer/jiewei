...
class controllerInit extends filterInit{
	...
	public function check_token($ispost = true) {
		...
		if ($this->get_gp('yunsoft_token') != $this->get_token()) return false;
		return true;
	}
	...
	private function set_token() {
		if (!$_COOKIE['yunsoft_token']) {
			$str = substr(md5(time(). $this->get_useragent()), 5, 8);
			setcookie("yunsoft_token", $str, NULL, '/','.zj3w.net');
			$_COOKIE['yunsoft_token'] = $str;
		}
	}
}
