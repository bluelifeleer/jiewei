...
//用户导入平台的商品
                ...
                $sales = intval(intval($getGoodsSales) + intval($value['goods_number']));
                //更新商品库存
                $upDataGoodsStock = InitPHP::getDao('product')->update(array('stock' => $stock), array('id' => $importGoodsInfo['id']));
                $upDataGoodsSales = InitPHP::getDao('product')->update(array('sales' => $sales), array('id' => $getGoodsDetail['id']));
