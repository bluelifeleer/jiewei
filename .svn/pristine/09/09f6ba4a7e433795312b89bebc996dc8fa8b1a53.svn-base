$(function() {
  ...
  $(document).on('pageInit', '#page-shop-order-infomartion', function(e, id, page) {
    ...
    var url = encodeURI(apiUrl + 'order/index/get/order_id/' + orderId+'/is_shop/1');
                ...
                $.each(orderdata['detail'][0], function(order_id, item) {
                  ...
                  switch (parseInt(item.goods_status)) {
                    ...
                    orderStatus = '付款成功，等待发货';
                      if(parseInt(item.from_id) > 1){
                            orderGoodsStatusHtml ='<a class="flr bor1 bce6 txac borrad5 marl10 padl10 padr10 c666 fs06"  href="delivery.php?order_id='+ item['order_id'] +'&pro_id='+item['goods_id']+'">发货</a>';
                         }
