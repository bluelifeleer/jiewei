...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        var choseText = $("<td/>");
        choseText.html('<a href="#" onclick="callback_dialog(this);" class="btn btn-default btn-lg callback" style="position: relative; z-index: 1;"><span>选择</span></a>');
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    if (res.url) {
        url = res.url;
        str = "<div><strong>资源地址:</strong><a href=" + res.url + " target='_blank' > " + res.url + "</a></div>" +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    } else {
        var domain = up.getOption('domain');
        url = domain + encodeURI(res.key);
        var link = domain + res.key;
        str = "<div><strong>资源地址:</strong><a href=" + url + " target='_blank' > " + link + "</a></div>" +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    }

    var td3 = this.fileProgressWrapper.find('td:eq(3) > a');
        td3.attr('uri',url);
    ...
    var showImg = $('<img src="resource/qiniu/loading.gif"/>');
