...
class AccessToken{
    ...
    private static function _getAccessToken(){
        ...
        define('CACHE_TOKEN_PATH', CACHE_PATH.'access_token'.DIRECTORY_SEPARATOR.WECHAT_APPID.DIRECTORY_SEPARATOR);
         if(!file_exists(CACHE_TOKEN_PATH.'access_token')){
            if(!is_dir(CACHE_TOKEN_PATH)) {
                    mkdir(CACHE_TOKEN_PATH, 0777, true);
                }
           @file_put_contents($CACHE_TOKEN_PATH.'access_token','');
        }
        $f = @fopen(CACHE_TOKEN_PATH.'access_token', 'w+');
...
//        $accessToken = YourDatabase::get('access_token');
        define('CACHE_TOKEN_PATH', CACHE_PATH.'access_token'.DIRECTORY_SEPARATOR.WECHAT_APPID.DIRECTORY_SEPARATOR);
        if(!file_exists(CACHE_TOKEN_PATH.'access_token'))return false;
        $data = @file_get_contents(CACHE_TOKEN_PATH.'access_token');
