...
class requestInit
{
    ...
    {
        if ($this->get_service('HTTP_X_REQUESTED_WITH') && strtolower($this->get_service('HTTP_X_REQUESTED_WITH')) == 'xmlhttprequest') {
            return true;
        }

        if ($this->get_post('initphp_ajax') || $this->get_get('initphp_ajax')) {
            return true;
        }
        //程序中自定义AJAX标识
        return false;
